...
- 左侧“项目候选人池”固定为当前项目的候选人池，不再随候选人进度节点切换成面邀、面评或 Offer 列表。
- 候选人进度节点改为右侧详情内的局部操作切换，点击 `筛选 / 面邀 / 面评 / Offer` 只切换当前候选人的操作区。
- 候选人进度节点增加完成勾选状态，已完成环节用勾选标识，当前环节保持高亮。
- 原分阶段批处理能力弱化为项目池筛选：`全部 / 待筛选 / 待面邀 / 待面评 / Offer跟进`。
- 本次不涉及后端 schema 或数据迁移；列表 API 仅补充院校、专业、学历等非敏感摘要字段用于候选人池展示。
- 项目候选人池列表将“来源”列调整为“院校”，以院校主信息 + 专业/学历辅助信息展示；长院校名称做省略和完整标题保留，避免窄屏挤压匹配分和当前阶段。
