...
if git grep --cached -n -E 'sk-[A-Za-z0-9_-]{16,}|APP_AUTH_TOKEN=[^[:space:]]{8,}|BAILIAN_API_KEY=sk-|MS_CLIENT_SECRET=[^[:space:]]{8,}|LARK_BASE_TOKEN=[A-Za-z0-9]{8,}|INTERVIEW_SPREADSHEET_TOKEN=[A-Za-z0-9]{8,}|LARK_NOTIFICATION_WEBHOOK_URL=https://open\.feishu\.cn/open-apis/bot/v2/hook/[A-Za-z0-9_-]{8,}' -- . ':!*.md' ':!.env.example' ':!scripts/publish.sh' >/tmp/leai-recruiting-secret-scan.log 2>/dev/null; then
